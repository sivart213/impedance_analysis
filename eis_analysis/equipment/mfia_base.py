...
class MFIABase:
    ...
    def child_node_list(self, path, as_nodes=False, recursive=True, leavesonly=True, **kwargs):
        ...
        if self.is_node(path):
            nodes = list(
                self.device[path].child_nodes(recursive=recursive, leavesonly=leavesonly, **kwargs)
            )
            if as_nodes:
                return nodes
    ...
    def async_get(self, path, key="value"):
        ...
        if self.is_node(path):
            ...
            if key in res:
                return (
                    res[key].item()  # type: ignore
                    if not isinstance(res[key], (float, int)) and len(res[key]) == 1  # type: ignore
                    else res[key]  # type: ignore
                )
                ...
                for v in res.values():
                    if key in v:
                        vres = (
                            v[key].item()
                            if not isinstance(v[key], (float, int)) and len(v[key]) == 1
                            else v[key]
                        )
                        kres.append(vres)
                if len(kres) == 1:
    ...
    def toggle_imps_module(self, set_to="on"):
        if self.device.imps[0].enable() == 1 and set_to in ["off", "toggle"]:
            self.device.imps[0].enable(0)

        elif self.device.imps[0].enable() == 0 and set_to in ["on", "toggle"]:
...
if __name__ == "__main__":
    ...
    mfia_base = MFIABase("192.168.94.86")

    # tc = Thermocouple(thermo_type="K", unit="C")

    # mV_conv = tc.find_pre_conversion(mfia_base.auxin1(), [50,90])

    # # tc_source = tc.wrap_source_function(mfia_base.auxin1, mV_conv)
    # tc_source = lambda : mV_conv(mfia_base.auxin1()) +1
