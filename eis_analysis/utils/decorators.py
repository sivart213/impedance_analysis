...
def recursive(func):
    ...
    def wrapper(arg, *args, **kwargs):
        if isinstance(arg, (tuple, list, set, np.ndarray)):
            ...
            return handle_dicts(wrapper)(arg, *args, **kwargs)
    ...
    def decorator(func):
        def wrapper(arg, *args, **kwargs):
            ...
            if any(t in accept_types for t in [int, float, np.integer, np.number]):
                ...
                if not isinstance(arg, tuple(accept_types)) and isinstance(arg, (int, float, np.integer, np.number)):
                    arg = (
                        float(arg)
                        if isinstance(arg, (int, np.integer))
                        else int(arg)
    ...
    def decorator(func):
        def wrapper(arg, *args, **kwargs):
            if isinstance(arg, tuple(accept_types)):
                ...
                raise TypeError(f"Argument of type {type(arg)} is not acceptable. Acceptable types: {accept_types}")
