...
class MFIAFileConverter(QMainWindow, WorkerFunctions):
    ...
    def __init__(self, **kwargs):
        super().__init__()
        # self.is_xlsx = True
        self.files: pd.DataFrame = pd.DataFrame()  # Initialize as an empty DataFrame
        self.in_path = None
        self.t_files = None
        ...
        # list Widgets
        self.file_kw_and_list = ListWidgetWithMenu(
            pattern_check_str="not ",
            # callbacks={
            #     "Reset": lambda: self.file_kw_and_list.reset_items(),
            #     "Sort": lambda: self.file_kw_and_list.sort_items(),
            # },
            has_checks=True,
            parent=self,
        )
        ...
        self.file_kw_or_list = ListWidgetWithMenu(
            pattern_check_str="not ",
            # callbacks={
            #     "Reset": lambda: self.file_kw_or_list.reset_items(),
            #     "Sort": lambda: self.file_kw_or_list.sort_items(),
            # },
            has_checks=False,
            parent=self,
        )
        ...
        pk_list += ["real", "Z'("] * 3
        pk_list += ["imag", "Z''("] * 2
        pk_list += ["imps", "abs", "phase"]

        self.columns_list = ColumnListWidget(
            preferred_keys=pk_list,
            pattern_check_str="use ",
            callbacks={
                "reset": self.reset_columns_list,
                "refresh": self.refresh_columns_list,
                "sort": self.sort_columns_list,
            },
            has_checks=True,
            parent=self,
        )
        ...
        # Removed buttons for self.columns_list and converted them into callbacks
        ...
        self.save_format_combo.addItems(["All Columns", "Selected Columns", "Freq, Real, & Imag"])
        ...
        # # Patterns and Keywords Layout
        # file_kw_layout = QHBoxLayout()
        # file_kw_layout.addWidget(self.file_kw_and_list)
        # file_kw_layout.addWidget(self.file_kw_or_list)
        # file_kw_frame = QFrame()
        # file_kw_frame.setLayout(file_kw_layout)
        # file_kw_frame.setMaximumWidth(max_width)

        # Patterns and Keywords Layout
        kw_and_layout = QVBoxLayout()
        kw_and_layout.addWidget(QLabel("<b>Search terms:</b> (all)"))
        kw_and_layout.addWidget(self.file_kw_and_list)
        kw_and_frame = QFrame()
        kw_and_frame.setLayout(kw_and_layout)
        kw_and_frame.setMaximumWidth(max_width)

        # Patterns and Keywords Layout
        kw_or_layout = QVBoxLayout()
        kw_or_layout.addWidget(QLabel("<b>Filter terms:</b> (any)"))
        kw_or_layout.addWidget(self.file_kw_or_list)
        kw_or_frame = QFrame()
        kw_or_frame.setLayout(kw_or_layout)
        kw_or_frame.setMaximumWidth(max_width)
        ...
        grid1.addWidget(kw_and_frame, 0, 0)
        grid1.addWidget(kw_or_frame, 0, 1)
        grid1.addWidget(self.group_tabs, 0, 2)
        ...
        datasets_widget_layout = QHBoxLayout()
        datasets_widget_layout.addWidget(self.tree)
        datasets_widget_layout.addWidget(columns_frame)

        datasets_layout.addLayout(datasets_widget_layout)
    ...
    def eventFilter(self, source, event):
        """Handles key press events for deleting items and context menus."""
        if event.type() == QEvent.KeyPress and event.key() == Qt.Key_Delete:
            if source is self.files_table:
                # Handle delete key for files_table
                rows_to_delete = set()
                ...
                return True
        elif event.type() == QEvent.ContextMenu and source is self.tree:
            # Handle context menu for tree
    ...
    def add_grouping_tab(self, search_terms="", reject_terms="", keys=""):
        ...
        search_terms = (
            ", ".join(search_terms) if isinstance(search_terms, list) else str(search_terms)
        )

        title = f"Group {self.group_tabs.count() + 1}"
        reject_terms = (
            ", ".join(reject_terms) if isinstance(reject_terms, list) else str(reject_terms)
        )
        ...
        if self.group_tabs.count() > 1:
            previous_tab = self.group_tabs.widget(self.group_tabs.count() - 2)
            previous_layout = previous_tab.layout().itemAt(previous_tab.layout().count() - 1)
            previous_layout.removeWidget(
                previous_layout.itemAt(previous_layout.count() - 1).widget()
            )
    ...
    def drop_grouping_tab(self):
        ...
        if n_tabs := self.group_tabs.count() > 1:
            ...
            if is_last_tab:
                ...
                last_layout = last_tab.layout().itemAt(last_tab.layout().count() - 1)
    ...
    def update_files(self):
        ...
        try:
            # Use get_list to retrieve patterns and apply re_not to checked items
            patterns = self.file_kw_and_list.get_list(transform_checked=re_not)

            # Use get_list to retrieve keywords
            keywords = self.file_kw_or_list.get_list() if self.file_kw_or_list.count() else None

            # Check if "Get All" checkbox is checked
            get_all = self.get_all_checkbox.isChecked()

            # Find files based on patterns
            files = find_files(self.in_path, patterns=patterns)
        ...
        self.files.insert(
            self.files.columns.get_loc("path"),  # type: ignore
            "relative path",
            ...
            for col_idx, value in enumerate(row):
                if "date" in self.files.columns[col_idx].lower():
                    self.files_table.setItem(
                        row_idx, col_idx, QTableWidgetItem(value.strftime("%Y-%m-%d"))
                    )
                elif "time" in self.files.columns[col_idx].lower():
                    self.files_table.setItem(
                        row_idx, col_idx, QTableWidgetItem(value.strftime("%H:%M:%S"))
                    )
                else:
    ...
    def update_t_files(self):
        ...
        for row in range(self.files_table.rowCount()):
            ...
            if name_item and path_item:
                ...
                if self.files.apply(
                    lambda x: x[name_str] == name_val and x[path_str] == path_val, axis=1
                ).any():
    ...
    def load_files(self):
        ...
        self.worker = LoadDatasetsWorker(
            self.t_files, self.tree, self.get_all_checkbox.isChecked()
        )
    ...
    def io_finished(self, loaded_data=None):
        ...
        assert self.progress_dialog is not None, "Progress dialog is not initialized."
    ...
    def group_by_tab(self, tab=None, suppress_refresh=False):
        ...
        if form_layout[2].text():
            ...
            reject_terms = form_layout[4].text().split(", ") if form_layout[4].text() else None
            keys = form_layout[6].text().split(", ") if form_layout[6].text() else None
            ...
            self.grouping_history.append(
                {"search_terms": search_terms, "reject_terms": reject_terms, "keys": keys}
            )
    ...
    def reapply_groups(self):
        ...
        for group in self.grouping_history:
            if group == "grouped_by_subname":
                ...
                self.tree.eval_grouping(
                    group["search_terms"], group["reject_terms"], group["keys"]
                )
    ...
    def reset_columns_list(self):
        """Delegate to ColumnListWidget."""
        self.columns_list.reset_columns_list(self.tree.data_org)

    def refresh_columns_list(self):
        """Delegate to ColumnListWidget."""
        self.columns_list.refresh_columns_list(self.tree.data_org)

    def sort_columns_list(self):
        """Delegate to ColumnListWidget."""
        self.columns_list.sort_columns_list()
    ...
    #         sel_col = get_valid_keys(columns, ["freq", "real", "imag"])
    #         if not sel_col:
    #             return columns
    #         return sel_col
    #     return columns
    ...
    def save_data(self, data=None, path=None):
        ...
        columns = self.columns_list.get_translated_columns(save_format == "Selected Columns")

        if data is None:
            data = self.tree.get_flat_data_org()

        flat_data = separate_dict(data, ["resid"])["residuals"]
    ...
    def save_settings(self):
        ...
        # Use get_list to retrieve patterns and keywords
        patterns = self.file_kw_and_list.get_list(transform_checked=lambda x: (x, True))
        keywords = self.file_kw_or_list.get_list()

        # Retrieve checked columns
        columns = [
    ...
    def load_settings(self):
        ...
        if not settings_file.exists():
            QMessageBox.warning(self, "No Settings", "Settings file not found.")
            return
        ...
        if directory_checkbox.isChecked():
            try:
                ...
                if settings["directory"] and path.exists():
                    self.in_path_label.setText(f"<b>Input Path:</b> {str(path)}")
                    self.in_path = path
            except KeyError:
                pass
        ...
        if patterns_checkbox.isChecked():
            try:
                if settings["patterns"]:
                    ...
                    for pattern, checked in settings["patterns"]:
                        self.file_kw_and_list.add_item(items=pattern)
                        item = self.file_kw_and_list.item(self.file_kw_and_list.count() - 1)
                        item.setCheckState(Qt.Checked if checked else Qt.Unchecked)

                if settings["keywords"]:
                    self.file_kw_or_list.clear()
                    self.file_kw_or_list.add_item(items=settings["keywords"])
        ...
        if columns_checkbox.isChecked():
            try:
                if settings["columns"]:
                    ...
                    self.columns_list.checked_columns = settings["columns"]
                    self.refresh_columns_list()
            except (KeyError, ValueError):
                pass
...
if __name__ == "__main__":
    ...
    #     # Used when compiling the "full" list of potential files
    #     self.file_kw_tabs.addTab(self.file_kw_and_list, "File Patterns")
    #     # Used when filtering the files to be loaded
    ...
    #     datasets_widget_layout = QHBoxLayout()
    #     datasets_widget_layout.addWidget(self.tree)
    #     datasets_widget_layout.addWidget(self.columns_frame)
